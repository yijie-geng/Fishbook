...
path <- "" # set path to a GMT format disease gene set library .csv file, such as the DisGeNET or GLAD4U library
...
for (i in 1:nrow(data)){
  ...
  length_cutoff = 250   # set length cutoff; 500 for DisGeNET, 250 for GLAD4U 
  if(length(x) < length_cutoff) next
  ...
  for (j in dbs) {
    ...
    write.csv(ordered_df, file = paste(as.character(j), " ", as.character(data[i,1]), ".csv", sep=""), row.names=FALSE)
  }
}
